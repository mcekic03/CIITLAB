...
async function displayResearchers(researchers) {
  ...
  researchersGrid.innerHTML = researchers
    .map(
      (researcher) => `
        <div class="researcher-card">
            ...
            <div class="researcher-info">
                ...
                <p class="email">${researcher.email}</p>
                <p class="researcher-excerpt">
                    ${researcher.bio || 'No biography available.'}
                </p>
...
// Function to fetch researchers
async function fetchResearchers() {
  try {
    const response = await fetch(`${API_BASE_URL}/getResearchers`);
    if (response.ok) {
    ...
    showError('An error occurred while loading researchers');
  }
}
...
// Function to create researcher profile page
function createResearcherProfile(researcher) {
  return `
        <div class="researcher-profile">
            <div class="profile-header">
                <div class="profile-image">
                    <img src="${researcher.image}" alt="${
    researcher.firstName
  } ${researcher.lastName}">
                </div>
                <div class="profile-info">
                    <h1>${researcher.firstName} ${researcher.lastName}</h1>
                    <p class="email">${researcher.email}</p>
                </div>
            </div>
...
// Function to load researcher profile
function loadResearcherProfile() {
  ...
  if (mainContent) {
    mainContent.innerHTML = createResearcherProfile(researcher);
  }
}
